...
select shift_gcarc,snr_scs,evlo,evla,stlo,stla,eq,stnm,hitlo,hitla from Master_a10 where wantit=1 order by eq,gcarc;
EOF
...
EOF
	...
	${EXECDIR}/AddDataNoise.out 1 4 3 << EOF
${AddNoise}
...
done < ${WORKDIR}/tmpfile_DataNoise_${RunNumber}
# Done Model loop.
