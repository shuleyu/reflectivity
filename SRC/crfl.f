...
c SY_CHANGE
      parameter(nfr=131072,npa=5000,nla=1000,ndis=900)
...
c
 ...
 104     if(iss(12).eq.1) zs(i)=6371.*alog(6371./(6371.-zs(i)))
c104     if(iss(12).eq.1) zs(i)=6371.*(1.-exp(-zs(i)/6371.))
         dso=zs(1)
         nso=1
         nre=1
         do 105 i=1,nlay
...
c
 ...
 606  format(//1x,'frequency window :'//1x,'low frequency cut at ',
     1f5.3,' hz'/1x,'high frequency cut at ',f5.3,' hz'/1x,'reference fr
     2equency =',f5.2,' hz'/1x,'exponent gamma for power law =',f5.2)
      if(iss(11).ne.0)write(6,607)iss(11),fu,fwil,iss(11),fwir,fo
 607  format(1x,'cos**',i2,' taper from ',f5.3,' hz to ',f5.3,' hz'/
     11x,'cos**',i2,' taper from ',f5.3,' hz to ',f5.3,' hz')
c
...
c
 ...
 3400 l1 = 2
      l2 = 2
 3500 sphred=1.
      if(iss(12).eq.1)  sphred=sqrt((x(ke)/6371.)/abs(sin(x(ke)/6371.)))
     1*((6371.-dso)/6371.)**(float(nrho+1)/2.)
      do 3600 kom = l1,l2
      go to(3610,3630),kom
...
c **********************************************************************
      ...
      if(iss(20).ne.1) then
         ...
         do  910  i=1,ii
 910     dso=dso+d(i)
         if(iss(12).eq.1) dso=6371.*(1.-exp(-dso/6371.))
 900     dre=0.
         ii=ire
