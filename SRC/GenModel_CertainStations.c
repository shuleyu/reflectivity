...
int main(int argc, char **argv){
    ...
    fprintf(fpout,"%10.4lf%10.4lf%10.4lf%10.4lf%10d\n",P[DISTMIN]*kmperdeg,P[DISTMAX]*kmperdeg,kmperdeg,0.0,filenr(PS[CertainStation]));

	// Read in Station distances and azimuths.
	int NPTS_ST=filenr(PS[CertainStation]);
	double *DIST=(double *)malloc(NPTS_ST*sizeof(double));
	double *AZ=(double *)malloc(NPTS_ST*sizeof(double));
    fptmp=fopen(PS[CertainStation],"r");
	for (count=0;count<NPTS_ST;count++){
		fscanf(fptmp,"%lf%lf",&DIST[count],&AZ[count]);
		DIST[count]*=kmperdeg;
		AZ[count]=(AZ[count]>360)?(AZ[count]-360):AZ[count];
	}
    fclose(fptmp);
    ...
    // Station distances.
    count=0;
    for (count2=0;count2<NPTS_ST;count2++){
		fprintf(fpout,"%10.4lf",DIST[count2]);
		count++;
		if (count%8==0){
			fprintf(fpout,"\n");
		}
    ...
    count=0;
    for (count2=0;count2<NPTS_ST;count2++){
		fprintf(fpout,"%10.4lf",AZ[count2]);
		count++;
		if (count%8==0){
			fprintf(fpout,"\n");
		}
...
// }
    ...
    while ( h<6171.0 ){
        ...
        if (h>P[ReflDepth] && flag2==0){
            ...
            printf("Velocity (after earth-flatening) in the layer right above refl. zone is:\n");
            printf("PREM  Vp / Vs: %.2lf / %.2lf \n",VP_refl,VS_refl);
            printf("Model Vp / Vs: %.2lf / %.2lf \n\n",VP_refl_model,VS_refl_model);
