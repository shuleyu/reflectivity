...
c    &unit21=a21,unit22=a22,unit23=a23)
      parameter(nfr=1024,npa=2500,nla=1000,ndis=200)
     ...
     &s(8193),wavel(4096),balla(8192),iballa(8192),window(npa),
     &phase1(nfr),sc1(4096)
...
c    &small arguments')
 ...
 698  format(//' max. number of ray parameters: ',i5/' max. number of fr
     &equencies: ',i5/' max. number of layers: ',i5/' max. number of dis
     &tancies: ',i5)
      call erdmod(iins,nh)
      if(iss(17).eq.1) then
c
...
c
 ...
 104     if(iss(12).eq.1) zs(i)=6370.*alog(6370./(6370.-zs(i)))
c104     if(iss(12).eq.1) zs(i)=6370.*(1.-exp(-zs(i)/6370.))
         dso=zs(1)
         nso=1
         nre=1
         do 105 i=1,nlay
...
c
      call matmul(rppd2r,rppd2i,rspd2r,rspd2i,rpsd2r,rpsd2i,rssd2r,
     ...
     &d12r,d12i,d21r,d21i,d22r,d22i,nfreq)
      call matmul(tppu1r,tppu1i,tspu1r,tspu1i,tpsu1r,tpsu1i,tssu1r,
...
c
     ...
     &a11r,a11i,a12r,a12i,a21r,a21i,a22r,a22i,nfreq)
      call matmul(tppu2r,tppu2i,tspu2r,tspu2i,tpsu2r,tpsu2i,tssu2r,
...
c
 1010 call matmul(tppu1r,tppu1i,tspu1r,tspu1i,tpsu1r,tpsu1i,tssu1r,
     &tssu1i,rppd2r,rppd2i,rspd2r,rspd2i,rpsd2r,rpsd2i,rssd2r,rssd2i,
     &a11r,a11i,a12r,a12i,a21r,a21i,a22r,a22i,nfreq)
      call matmul(ua1r,ua1i,ua2r,ua2i,ua3r,ua3i,ua4r,ua4i,a11r,a11i,
...
c
 1100 if(ire.ne.0.and.iss(21).eq.1) then
     ...
     &   b22r,b22i,b21r,b21i,nfreq)
         call matmul(b11r,b11i,b12r,b12i,b21r,b21i,b22r,b22i,c11r,c11i,
...
c
      call matmul(rppd1r,rppd1i,rspd1r,rspd1i,rpsd1r,rpsd1i,rssd1r,
...
c
      call matmul(b11r,b11i,b12r,b12i,b21r,b21i,b22r,b22i,tppu1r,tppu1i,
     &tspu1r,tspu1i,tpsu1r,tpsu1i,tssu1r,tssu1i,d11r,d11i,d12r,d12i,
     &d21r,d21i,d22r,d22i,nfreq)
      call matmul(d11r,d11i,d12r,d12i,d21r,d21i,d22r,d22i,c11r,c11i,c12r
...
c
     ...
     &tppu2i,tspu2r,tspu2i,tssu2r,tssu2i,tpsu2r,tpsu2i,0)
      call matmul(rppu1r,rppu1i,rspu1r,rspu1i,rpsu1r,rpsu1i,rssu1r,
...
c
 1310 call matmul(ua1r,ua1i,ua2r,ua2i,ua3r,ua3i,ua4r,ua4i,rppu1r,rppu1i,
...
c
      call matmul(c11r,c11i,c12r,c12i,c21r,c21i,c22r,c22i,a11r,a11i,
     &a12r,a12i,a21r,a21i,a22r,a22i,y11r,y11i,y12r,y12i,y21r,y21i,y22r,
     &y22i,nfreq)
c
...
c                            - x1
c
      call matmul(y11r,y11i,y12r,y12i,y21r,y21i,y22r,y22i,rppd2r,rppd2i,
...
c
 1320 call matmul(ub1r,ub1i,ub2r,ub2i,ub3r,ub3i,ub4r,ub4i,rppu1r,rppu1i,
     &rspu1r,rspu1i,rpsu1r,rpsu1i,rssu1r,rssu1i,b11r,b11i,b12r,b12i,
     &b21r,b21i,b22r,b22i,nfreq)
c
...
c                            - y1
c
      call matmul(x11r,x11i,x12r,x12i,x21r,x21i,x22r,x22i,rppd2r,rppd2i,
...
c
      call matmul(ub1r,ub1i,ub2r,ub2i,ub3r,ub3i,ub4r,ub4i,rppu1r,rppu1i,
     &rspu1r,rspu1i,rpsu1r,rpsu1i,rssu1r,rssu1i,b11r,b11i,b12r,b12i,
     &b21r,b21i,b22r,b22i,nfreq)
      call matadd(ua1r,ua1i,ua2r,ua2i,ua3r,ua3i,ua4r,ua4i,b11r,b11i,b12r
     &,b12i,b21r,b21i,b22r,b22i,c11r,c11i,c12r,c12i,c21r,c21i,c22r,c22i,
...
c
      call matmul(ua1r,ua1i,ua2r,ua2i,ua3r,ua3i,ua4r,ua4i,a11r,a11i,a12r
...
c
 1340 call matmul(ua1r,ua1i,ua2r,ua2i,ua3r,ua3i,ua4r,ua4i,rppu1r,rppu1i,
     &rspu1r,rspu1i,rpsu1r,rpsu1i,rssu1r,rssu1i,b11r,b11i,b12r,b12i,
     &b21r,b21i,b22r,b22i,nfreq)
      if(iss(16).eq.1.and.mdeck.ge.ire) then
         call matmul(b11r,b11i,b12r,b12i,b21r,b21i,b22r,b22i,a11r,a11i,
...
c
 ...
 3400 l1 = 2
      l2 = 2
 3500 sphred=1.
      if(iss(12).eq.1)  sphred=sqrt((x(ke)/6370.)/abs(sin(x(ke)/6370.)))
     1*((6370.-dso)/6370.)**(float(nrho+1)/2.)
      do 3600 kom = l1,l2
      go to(3610,3630),kom
...
c
      parameter(nfr=1024,npa=2500,nla=1000)
...
330      x4i(i)=tpsdi*z1r(i)+tpsdr*z1i(i)
      ...
      call matmul(x1r,x1i,x2r,x2i,x4r,x4i,x3r,x3i,tppr,tppi,tspr,tspi,
...
340   continue
300   continue
      call matmul(xppr,xppi,xspr,xspi,xpsr,xpsi,xssr,xssi,x11r,x11i,x12r
...
216   continue
      ...
      x4r(i)=rssur
 211  x4i(i)=rssui
      call matmul(x1r,x1i,x2r,x2i,x3r,x3i,x4r,x4i,rppr,rppi,rspr,rspi,
     ...
     &x3i,x4r,x4i,nfreq)
      ...
      w4r(i)=tssdr
 212  w4i(i)=tssdi
      call matmul(x1r,x1i,x2r,x2i,x3r,x3i,x4r,x4i,w1r,w1i,w2r,w2i,w3r,
     &w3i,w4r,w4i,z1r,z1i,z2r,z2i,z11r,z11i,z22r,z22i,nfreq)
      call matmul(tppr,tppi,tspr,tspi,tpsr,tpsi,tssr,tssi,z1r,z1i,z2r,
...
c
 ...
 100    h22i(i)=aimag(l22)
        ...
        subroutine matmul(a11r,a11i,a12r,a12i,a21r,a21i,a22r,a22i,
 ...
 100    continue
     ...
     *  b11r,b11i,b12r,b12i,b21r,b21i,b22r,b22i,n)
        parameter(npa=2500,nfr=1024,nla=1000)
        dimension a11r(n),a11i(n),a12r(n),a12i(n),a21r(n),a21i(n),
     *  a22r(n),a22i(n),
     *  b11r(n),b11i(n),b12r(n),b12i(n),b21r(n),b21i(n),b22r(n),b22i(n),
...
c **********************************************************************
      ...
      if(iss(20).ne.1) then
         ...
         do  910  i=1,ii
 910     dso=dso+d(i)
         if(iss(12).eq.1) dso=6370.*(1.-exp(-dso/6370.))
 900     dre=0.
         ii=ire
...
9010  format(///,1x,'inhomogeneous velocity-depth distribution :'//,4x,
     ...
     1npkt)
 9020 format(1x,f10.4,10x,5f10.4,i10)
